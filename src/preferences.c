...
Preferences* preferences_load() {
    ...
    if(persist_exists(PREF_TEMP_FORMAT_PERSIST_KEY)) {
        prefs.temp_format = persist_read_int(
            PREF_TEMP_FORMAT_PERSIST_KEY
        );
    }
    if(persist_exists(PREF_WEATHER_UPDATE_FREQ_PERSIST_KEY)) {
        prefs.weather_update_freq = persist_read_int
            (PREF_WEATHER_UPDATE_FREQ_PERSIST_KEY
        );
    }
    if(persist_exists(PREF_STATUSBAR_PERSIST_KEY)) {
        prefs.statusbar = persist_read_bool(
            PREF_STATUSBAR_PERSIST_KEY
        );
    }
    if(persist_exists(PREF_WEATHER_PROVIDER_PERSIST_KEY)) {
        prefs.weather_provider = persist_read_int(
            PREF_WEATHER_PROVIDER_PERSIST_KEY
        );
    }
    if(persist_exists(PREF_WEATHER_OUTDATED_TIME_PERSIST_KEY)) {
        prefs.weather_outdated_time = persist_read_int(
            PREF_WEATHER_OUTDATED_TIME_PERSIST_KEY
        );
    }
    if(persist_exists(PREF_LANGUAGE_CODE_PERSIST_KEY)) {
        prefs.language_code = persist_read_int(
            PREF_LANGUAGE_CODE_PERSIST_KEY
        );
    }
    if(persist_exists(PREF_TRANSLATION_PERSIST_KEY)) {
        persist_read_string(
            PREF_TRANSLATION_PERSIST_KEY,
            prefs.translation, sizeof(prefs.translation)
        );
    }
...
bool preferences_save(Preferences* prefs) {
    status_t temp_format = persist_write_int(
        PREF_TEMP_FORMAT_PERSIST_KEY,
        prefs->temp_format
    );
    status_t weather_update_freq = persist_write_int(
        PREF_WEATHER_UPDATE_FREQ_PERSIST_KEY,
        (int)prefs->weather_update_freq
    );
    status_t statusbar = persist_write_bool(
        PREF_STATUSBAR_PERSIST_KEY,
        prefs->statusbar
    );
    status_t weather_provider = persist_write_int(
        PREF_WEATHER_PROVIDER_PERSIST_KEY,
        prefs->weather_provider
    );
    status_t weather_outdated_time = persist_write_int(
        PREF_WEATHER_OUTDATED_TIME_PERSIST_KEY,
        (int)prefs->weather_outdated_time
    );
    status_t language_code = persist_write_int(
        PREF_LANGUAGE_CODE_PERSIST_KEY,
        prefs->language_code
    );
    status_t translation = persist_write_string(
        PREF_TRANSLATION_PERSIST_KEY,
        prefs->translation
    );
    ...
    if(
        temp_format < 0
        || weather_update_freq < 0
        || statusbar < 0
        || weather_provider < 0
        || weather_outdated_time < 0
        || language_code < 0
        || translation < 0
    ) {
...
void preferences_send(Preferences* prefs) {
    ...
    Tuplet request = TupletInteger(
        SET_PREFERENCES_MSG_KEY,
        1
    );
    Tuplet temp_format = TupletInteger(
        TEMP_FORMAT_MSG_KEY,
        prefs->temp_format
    );
    Tuplet weather_update_frequency = TupletInteger(
        WEATHER_UPDATE_FREQ_MSG_KEY,
        (int)prefs->weather_update_freq
    );
    Tuplet statusbar = TupletInteger(
        STATUSBAR_MSG_KEY,
        prefs->statusbar ? 1 : 0
    );
    Tuplet weather_provider = TupletInteger(
        WEATHER_PROVIDER_MSG_KEY,
        prefs->weather_provider
    );
    Tuplet weather_outdated_time = TupletInteger(
        WEATHER_OUTDATED_TIME_MSG_KEY,
        (int)prefs->weather_outdated_time
    );
    Tuplet language_code = TupletInteger(
        LANGUAGE_CODE_MSG_KEY,
        prefs->language_code
    );
...
void preferences_set(Preferences* prefs, DictionaryIterator* iter) {
    Tuple* temp_format = dict_find(
        iter, TEMP_FORMAT_MSG_KEY
    );
    Tuple* weather_update_frequency = dict_find(
        iter, WEATHER_UPDATE_FREQ_MSG_KEY
    );
    Tuple* statusbar = dict_find(
        iter, STATUSBAR_MSG_KEY
    );
    Tuple* weather_provider = dict_find(
        iter, WEATHER_PROVIDER_MSG_KEY
    );
    Tuple* weather_outdated_time = dict_find(
        iter, WEATHER_OUTDATED_TIME_MSG_KEY
    );
    Tuple* language_code = dict_find(
        iter, LANGUAGE_CODE_MSG_KEY
    );
    Tuple* translation = dict_find(
        iter, TRANSLATION_MSG_KEY
    );
