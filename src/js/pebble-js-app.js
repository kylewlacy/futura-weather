function Preferences() {
    ...
    this.languageCode = 0;
    this.translation = "Jan,Feb,Mar,Apr,May,Jun,"
...
function Weather(temperature, conditions, lastUpdate) {
    this.temperature = valueOrDefault(temperature, -1);
    ...
    this.asMessage = function() {
        ...
        // TODO: Better flag handling (don't use
        // ternary conditional for `isDay` flag)
        return {
            "temperature": Math.round(weather.temperature * 100),
            "weatherConditionCode": weather.conditions,
            "weatherConditionFlags": isDay ? 1 : 0,
            "setWeather": 1
        };
    }
}
...
function YahooWeatherProvider() {
    ...
    var updateWeatherWithWoeidCallback = function(req, coords) {
        if (req.status == 200) {
            ...
            console.warn(
                "Error getting weather with Yahoo Weather "
                    + "(status " + req.status + ")"
            );
        }
    }
    ...
    var updateWeatherWithWoeid = function(req, weather, coords, callback) {
        ...
        makeRequest("GET", url, function(req) {
            weather.setFromJsonObject(
                updateWeatherWithWoeidCallback(req, coords)
            );
            if(typeof(callback) === "function") { callback(weather); }
        });
    }
    ...
    var woeidCallback = function(req, weather, coords, callback) {
        if(req.status == 200) {
            ...
            console.warn(
                "Error getting Yahoo Weather WOEID (status "
                    + req.status + ")"
            );
        }
    }
...
var configURL =
    "http://kylewlacy.github.io/futura-weather-redux/v3/preferences.html";
...
function fetchWeather() {
    if(
        ...
        console.warn(
            "Weather update requested too soon; loading from cache "
                + "(" + (new Date()).toString() + ")"
        );
        sendWeather(weather);
    }
